...
*         : 20.09.2022 5.40    Updated demo project:
*                              Call R_CAN_Control(ch_nr, OPERATE_CANMODE) before call R_CAN_TxSet() or R_CAN_TxSetXid().
***********************************************************************************************************************/
/***********************************************************************************************************************
...
************************************************************************************************************************/
/* Demo data */
can_frame_t  tx_dataframe, rx_dataframe, remote_frame;
...
void main(void)
{
    ...
    *****************************************/
    ...
    while(1)
    {
        ...
        if (api_status == R_CAN_OK)
        {
        	R_CAN_TxSet(g_can_channel, g_mb_mode, CANBOX_TX, &tx_dataframe, DATA_FRAME);
        }
        R_BSP_SoftwareDelay(100u, BSP_DELAY_MILLISECS);
        can_poll_demo();

		/* Check CAN tx data matches rx data */
		if (tx_dataframe.data[0] == rx_dataframe.data[0])
		{
	        LED0 = LED_ON;
			LED1 = LED_ON;
			LED2 = LED_ON;
			LED3 = LED_ON;
			R_BSP_SoftwareDelay(800u, BSP_DELAY_MILLISECS);
			LED0 = LED_OFF;
			LED1 = LED_OFF;
			LED2 = LED_OFF;
			LED3 = LED_OFF;
			R_BSP_SoftwareDelay(200u, BSP_DELAY_MILLISECS);
		}
		else
		{
	        LED0 = LED_ON;
